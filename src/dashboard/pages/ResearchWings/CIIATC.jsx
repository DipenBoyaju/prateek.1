...
const CIIATC = () => {
  ...
  const symbol = research && Array.isArray(research) && research.length > 0 ? research[3].symbol : null;

  // Update local content state when data is loaded
  useEffect(() => {
    if (research && research[3]) {
      setContent(research[3].description);
    }
  }, [research]);
    ...
    <div>
      <div className="bg-white p-4 flex justify-between shadow rounded-md">
        <p className="text-sm text-zinc-800/90 flex items-center gap-1">
          ...
          <span className="text-blue-500">{research[3]?.symbol}</span>
        </p>
        <button
      ...
      <div className="my-5 bg-white p-4 rounded-md ">
        <div className="">
          <h1 className="text-xl md:text-2xl text-blue-600 font-semibold">{research[3]?.title}</h1>
          <p className="pt-3 font-poppins font-light text-sm">{research[3]?.description}</p>
        </div>
