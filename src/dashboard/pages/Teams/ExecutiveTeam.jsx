    ...
    <div className="max-w-full">
      <div className="bg-white p-4 flex justify-between shadow rounded-md">
        <p className="text-sm text-zinc-800/90 flex items-center">
          Team <HiMiniSlash className="text-base" />{' '}
          <span className="text-blue-500">Executive</span>
        </p>
        <div>
          <button
            onClick={() => nav('/dashboard/team/addmember')}
      ...
      <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-5">
        {cardLayout ? (
          team?.map((member) => <DasTeamCard key={member?._id} member={member} />)
        ) : (
              ...
              <table className="min-w-full divide-y divide-zinc-200">
                <thead className="bg-blue-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-sm font-semibold text-zinc-600 text-left">Name</th>
